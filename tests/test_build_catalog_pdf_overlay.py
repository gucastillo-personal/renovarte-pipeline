...
descuento de oferta, siempre con un piso de costo+margen (nunca por debajo
— ver test de regresión abajo); códigos sin precio en el PDF siguen con
costo+margen, sin cambios de comportamiento.
"""
...
def test_pdf_price_below_costo_mas_margen_floor_is_ignored(tmp_path: Path) -> None:
    """Regression: real LACA data has Precio ABC == Precio Profesional for
    ~1 in 3 products (their cost to RenovArte) — never sell at that price
    outright, the configured margin always applies as a floor.
    """
    result = build_catalog(
        [_row()],
        env={"MARGIN_PERCENT_DEFAULT": "20"},  # floor: 1200
        out_path=tmp_path / "products.json",
        pdf_prices={"001": 1000},  # e.g. ABC == costo, no LACA markup baked in
    )
    assert result.products[0].precio_venta == 1200


def test_pdf_price_below_floor_and_offer_combined(tmp_path: Path) -> None:
    result = build_catalog(
        [_row()],
        env={"MARGIN_PERCENT_DEFAULT": "20"},  # floor: 1200
        out_path=tmp_path / "products.json",
    ...
    product = result.products[0]
    assert product.precio_regular == 1200  # the floor, not the PDF's 1000
